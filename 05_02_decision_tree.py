...
# 4. Используется sklearn Pipeline (scaler + модель едины)
# 5. убраны абсолютные пути — используется config.py
# 6. Сохранение Pipeline целиком (без отдельного scaler)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(DATA_DIR, "decision_tree.log"), encoding="utf-8"
        ),
...
pipeline_path = os.path.join(MODELS_DIR, "decision_tree_best.pkl")
joblib.dump(best_pipeline, pipeline_path)
log.info(f"   ✅ Pipeline: {pipeline_path}")

importance_path = os.path.join(MODELS_DIR, "decision_tree_feature_importance.csv")
...
metrics_path = os.path.join(MODELS_DIR, "decision_tree_metrics.csv")
...
plt.tight_layout()
plot_path = os.path.join(MODELS_DIR, 'decision_tree_diagnostics.png')
