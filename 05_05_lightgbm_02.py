# 05_05_lightgbm.py
# lightgbm для бинарной классификации риска расторжения контракта.
# структура идентична остальным ml-скриптам:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(DATA_DIR, f"lightgbm{MODEL_SUFFIX}.log"), encoding="utf-8"
        ),
        logging.StreamHandler()
    ]
)
log = logging.getLogger("lightgbm")

log.info("=" * 60)
log.info("ЭТАП 5.5: LIGHTGBM (С КРОСС-ВАЛИДАЦИЕЙ)")
log.info("=" * 60)

# загрузка данных
log.info("\n1. загрузка данных...")

X = pd.read_csv(os.path.join(DATA_DIR, f"X_features_{DATASET_VERSION}.csv"))
y = pd.read_csv(os.path.join(DATA_DIR, f"y_target_{DATASET_VERSION}.csv")).values.ravel()

with open(os.path.join(DATA_DIR, f"feature_columns_{DATASET_VERSION}.txt"), "r", encoding="utf-8") as f:
...
# scale_pos_weight — нативный способ балансировки в lightgbm
...
# gridsearchcv
# lightgbm не требует масштабирования, но pipeline используем
...
pipe = Pipeline([
    ...
    ('clf', LGBMClassifier(
        ...
        random_state=42,
        n_jobs=-1,
        verbosity=-1,           # подавляем вывод lightgbm
        scale_pos_weight=scale_pos_weight
    ))
])

# сетка параметров специфична для lightgbm:
# num_leaves — основной параметр сложности (вместо max_depth)
# min_child_samples — аналог min_samples_leaf, важен для регуляризации
...
# lightgbm имеет встроенную важность (split, gain),
# но permutation importance честнее для сравнения с другими моделями
...
pipeline_path = os.path.join(MODELS_DIR, f"lightgbm{MODEL_SUFFIX}_best.pkl")
joblib.dump(best_pipeline, pipeline_path)
log.info(f"   pipeline: {pipeline_path}")

imp_path = os.path.join(MODELS_DIR, f"lightgbm_feature_importance{MODEL_SUFFIX}.csv")
imp_df.to_csv(imp_path, index=False)
log.info(f"   важность признаков: {imp_path}")

metrics_df = pd.DataFrame([{
    'model':                'LightGBM (best pipeline)',
...
metrics_path = os.path.join(MODELS_DIR, f"lightgbm_metrics{MODEL_SUFFIX}.csv")
...
fig.suptitle('LightGBM: диагностика', fontsize=16, fontweight='bold')

# roc-кривая
ax = axes[0, 0]
fpr, tpr, _ = roc_curve(y_test, y_proba)
ax.plot(fpr, tpr, lw=2.5, label=f'LightGBM (AUC={roc_auc:.3f})')
...
ax.plot(rec_curve, prec_curve, lw=2.5, label=f'LightGBM (F1={f1:.3f})')
...
plt.tight_layout()
plot_path = os.path.join(MODELS_DIR, f"lightgbm_diagnostics{MODEL_SUFFIX}.png")
plt.savefig(plot_path, dpi=150)
log.info(f"   график: {plot_path}")
plt.close()

log.info("\n" + "=" * 60)
log.info("ИТОГИ LIGHTGBM")
log.info("=" * 60)
