# дерево решений с кросс-валидацией и pipeline
...
DATASET_VERSION = "problematic_only"
MODEL_SUFFIX = "_problematic_only"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(DATA_DIR, f"decision_tree{MODEL_SUFFIX}.log"), encoding="utf-8"
        ),
...
log.info("=" * 60)
log.info("ЭТАП 5.2: ДЕРЕВО РЕШЕНИЙ (С КРОСС-ВАЛИДАЦИЕЙ)")
log.info("=" * 60)
...
X_path = os.path.join(DATA_DIR, f"X_features_{DATASET_VERSION}.csv")
y_path = os.path.join(DATA_DIR, f"y_target_{DATASET_VERSION}.csv")
features_path = os.path.join(DATA_DIR, f"feature_columns_{DATASET_VERSION}.txt")
...
# примечание: дерево решений не требует масштабирования,
# но pipeline с standardscaler используем для единообразия
# и корректной работы GridSearchCV (scaler fit только на train)
dummy_pipe = Pipeline([
...
# используется Pipeline(scaler + DecisionTree):
# - scaler обеспечивает корректность при CV (fit только на train)
# - параметры пайплайна задаются через clf__<param>
...
# вАЖНОСТЬ ПРИЗНАКОВ (через best_pipeline['clf'])
log.info("\n10. Анализ важности признаков...")

best_clf = best_pipeline.named_steps['clf']
importance_df = pd.DataFrame({
    "Признак": feature_names,
    "Важность": best_clf.feature_importances_
}).sort_values("Важность", ascending=False)

log.info("\n   Топ-15 наиболее важных признаков:")
for _, row in importance_df.head(15).iterrows():
    log.info(f"      {row['Признак']:35s}: {row['Важность']:.4f}")
...
pipeline_path = os.path.join(MODELS_DIR, f"decision_tree{MODEL_SUFFIX}_best.pkl")

joblib.dump(best_pipeline, pipeline_path)
log.info(f"   ✅ Pipeline: {pipeline_path}")

importance_path = os.path.join(MODELS_DIR, f"decision_tree_feature_importance{MODEL_SUFFIX}.csv")

importance_df.to_csv(importance_path, index=False)
log.info(f"   ✅ Важность признаков: {importance_path}")

metrics_dict = {
    'model':            'DecisionTree (best pipeline)',
...
metrics_path = os.path.join(MODELS_DIR, f"decision_tree_metrics{MODEL_SUFFIX}.csv")
...
fig.suptitle('Дерево решений: Диагностика', fontsize=16, fontweight='bold')

# roc-кривая
ax = axes[0, 0]
fpr, tpr, _ = roc_curve(y_test, y_proba)
ax.plot(fpr, tpr, lw=2.5, label=f'DecisionTree (AUC={roc_auc:.3f})')
...
ax.plot(recall_curve, precision_curve, lw=2.5, label=f'DecisionTree (F1={f1:.3f})')
...
# важность признаков
ax = axes[1, 1]
top15 = importance_df.head(15).sort_values("Важность")
ax.barh(top15["Признак"], top15["Важность"], color='#1e40af')
ax.set_xlabel('Важность')
ax.set_title('Топ-15 важных признаков')
ax.grid(True, alpha=0.3, axis='x')

plt.tight_layout()
plot_path = os.path.join(MODELS_DIR, f'decision_tree_diagnostics{MODEL_SUFFIX}.png')
...
plot_tree(
    ...
    class_names=['Успех', 'Расторжение'],
    filled=True,
    rounded=True,
    max_depth=3,
    fontsize=8
)
plt.title(f'Фрагмент дерева решений (глубина ≤ 3): {gs.best_params_}')
tree_path = os.path.join(MODELS_DIR, f"decision_tree_structure{MODEL_SUFFIX}.png")
plt.savefig(tree_path, dpi=150, bbox_inches='tight')
log.info(f"   ✅ Схема дерева: {tree_path}")
plt.close()

# итоги
log.info("\n" + "=" * 60)
log.info("ИТОГИ ДЕРЕВА РЕШЕНИЙ")
log.info("=" * 60)
...
log.info(f"\n✅ ЭТАП 5.2 ЗАВЕРШЁН — Pipeline сохранён в {pipeline_path}")
log.info("=" * 60)