...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(DATA_DIR, "create_ml_dataset_01.log"), encoding="utf-8"
        ),
...
DATASET_VERSION = "problematic_only"  # второй вариант: без соглашения сторон
...
df = pd.read_sql_query("""
    ...
    stage = 'EC'
    OR reg_num NOT IN (
        SELECT DISTINCT pf.reg_num
        FROM procedures_flat pf
        WHERE pf.termination_reason_name = 'Соглашение сторон'
    )
)
""", conn)
...
full_path     = os.path.join(DATA_DIR, f"ml_dataset_{DATASET_VERSION}.csv")
