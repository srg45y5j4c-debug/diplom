# 05_06_catboost.py
# catboost для бинарной классификации риска расторжения контракта.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(DATA_DIR, "catboost.log"), encoding="utf-8"
        ),
        logging.StreamHandler()
    ]
)
log = logging.getLogger("catboost")

log.info("=" * 60)
log.info("ЭТАП 5.6: CATBOOST (С КРОСС-ВАЛИДАЦИЕЙ)")
log.info("=" * 60)
...
# catboost использует scale_pos_weight аналогично xgboost/lightgbm
...
# gridsearchcv
# catboost не требует масштабирования и хорошо работает с сырыми данными,
# но pipeline используем для единообразия сохранения и инференса
...
pipe = Pipeline([
    ...
    ('clf', CatBoostClassifier(
        ...
        eval_metric='F1',
        random_seed=42,
        verbose=0,              # подавляем вывод catboost
        scale_pos_weight=scale_pos_weight,
        thread_count=-1
    ))
])

# сетка специфична для catboost:
# depth — глубина деревьев (catboost рекомендует 4-10)
# l2_leaf_reg — l2 регуляризация листьев (аналог lambda в xgboost)
...
# catboost имеет встроенную важность (shap-значения, feature importance),
# но для сравнения с другими моделями используем permutation importance
...
pipeline_path = os.path.join(MODELS_DIR, "catboost_best.pkl")
joblib.dump(best_pipeline, pipeline_path)
log.info(f"   pipeline: {pipeline_path}")

imp_path = os.path.join(MODELS_DIR, "catboost_feature_importance.csv")
imp_df.to_csv(imp_path, index=False)
log.info(f"   важность признаков: {imp_path}")

metrics_df = pd.DataFrame([{
    'model':                'CatBoost (best pipeline)',
...
metrics_path = os.path.join(MODELS_DIR, "catboost_metrics.csv")
...
fig.suptitle('CatBoost: диагностика', fontsize=16, fontweight='bold')

# roc-кривая
ax = axes[0, 0]
fpr, tpr, _ = roc_curve(y_test, y_proba)
ax.plot(fpr, tpr, lw=2.5, label=f'CatBoost (AUC={roc_auc:.3f})')
...
ax.plot(rec_curve, prec_curve, lw=2.5, label=f'CatBoost (F1={f1:.3f})')
...
plt.tight_layout()
plot_path = os.path.join(MODELS_DIR, 'catboost_diagnostics.png')
plt.savefig(plot_path, dpi=150)
log.info(f"   график: {plot_path}")
plt.close()

log.info("\n" + "=" * 60)
log.info("ИТОГИ CATBOOST")
log.info("=" * 60)
